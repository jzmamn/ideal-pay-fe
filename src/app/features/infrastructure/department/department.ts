...
import { Department as DepartmentEntity, MasterEntity } from '../../../shared/models/master-data.models';
...
export class Department {
  ...
  readonly data = this.masterSvc.departments;

  readonly config: MasterDataTableConfig<DepartmentEntity> = {
    title: 'Departments',
    showNewButton: true,
  ...
  constructor() {
    this.masterSvc.reload('departments');
  }
  ...
  openDialog(item?: MasterEntity): void {
    this.dialog.open(MasterDataDialog, {
      ...
      width: '600px',
      data: { entity: 'departments', title: 'Department', icon: 'corporate_fare', item, extraFields: EXTRA_FIELDS } satisfies MasterDataDialogData,
    });
  }
}
